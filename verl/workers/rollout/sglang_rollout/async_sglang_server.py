...
# Compatibility shim for sglang < 0.5.8 which lacks pause/continue generation API
try:
    from sglang.srt.managers.io_struct import (
...
class SGLangHttpServer:
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # Only enable sglang LoRA mode when using LoRA as an adapter at inference time.
        # When model.lora.merge=True, LoRA is merged into base weights before sync, so
        # sglang should run as a normal full-model engine (no adapter registration).
    ...
    async def generate(
        ...
        # Add lora request: only when running LoRA as an adapter.
        # In merge mode, the merged base weights are live on the engine, so we must not
        # request an adapter (none is registered).
        if self.lora_as_adapter:
    ...
    async def abort_all_requests(self):
        if hasattr(self.tokenizer_manager, "pause_generation"):
            await self.tokenizer_manager.pause_generation(PauseGenerationReqInput(mode="abort"))

    async def resume_generation(self):
        if hasattr(self.tokenizer_manager, "continue_generation"):
